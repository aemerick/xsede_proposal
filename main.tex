...
so a simulation run for 10~Myr requires on the order of $10^6$ steps. This leads to an estimate of total number of CPU hours of $3.49 \times 10^6$. Conducting this same estimate without winds means the highest temperature on the grid is generally due to the radiation at $10^4$ K (this anticipates that supernova on the high density grid are brief and infrequent events in our simulation). This leads to a total of $4.38 \times 10^5$ hrs for a single run. We therefore propose to do two runs for comparison, one with winds and one without.
