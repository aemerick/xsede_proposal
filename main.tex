...
This renewal proposal describes our proposed research for the coming year, focusing on using a new, detailed model for stellar feedback physics to understand the dynamical and chemical evolution of low mass dwarf galaxies. This project is a novel endeavor in galaxy-scale simulations as our method uses particles to trace individual stars in our galaxy, rather than whole stellar populations. This allows us to follow the feedback physics of individual stars in unprecedented detail -- including models for stellar winds, supernovae, diffusive stellar heating, and radiative transfer -- with high grid resolution ($\sim$ 1.5 pc). In addition, this new formalism allows us to trace stellar yields from individual stars, allowing us to trace stochastic variations in local and galactic scale chemical abundances in over a dozen different elements. The PI of this proposal (M-M. Mac Low) is PI on NSF grant AST11-09395 and co-PI Emerick is supported by a NSF Graduate Research Fellowship (DGE-16-44869); this project is co-advised by collaborator G. Bryan. We summarize the science motivation for each of these projects below, while justifying our resource request. We note that each of this project is central to the PhD thesis of co-PI Emerick.
...
The following constitutes the core set of physical processes used in our simulations that will be hold fixed in each simulation: 1) gas self-gravity to capture gas collapse and subsequent star formation in dense regions, 2) a nine-species, non-equilibrium chemistry solver, following electrons, and H, He, and H$_{2}$ and their ionization states \citep{Anninos1997, Abel1997}, 3) associated heating / cooling as followed in the \texttt{Grackle} library (Smith et al.\ in prep), including heating from a metagalactic UV background \citep{HM2012} and tabulated metal line cooling from \texttt{Cloudy} \citep{Cloudy2013}, 4) approximate H and He gas self-shielding of the UV background \citep{Rahmati2013}, 5) a metagalactic Lyman-Werner (LW) flux from \cite{HM2012} to properly model H$_{2}$ dissociation, and 6) a collisionless N-body solver to evolve the dynamics of the formed star particles. In every simulation, we form stars from dense gas on the most refined parts of the grid (with resolution of 1.5~pc) following a stochastic star formation scheme from \cite{Goldbaum2015, Goldbaum2016}, modified to sample individual stellar masses from an input IMF \citep[e.g.][]{Salpeter1955}. We use a high star gas density threshold for star formation, $n > 10^{3}$ cm$^{-3}$, and do not employ a star formation efficiency as a free parameter in order to test whether or not these feedback processes are capable of self-regulating star formation to the expected efficiency of $\sim$ 2\%; this is similar to what has been done in the FIRE feedback simulations \citep[e.g][]{Hopkins2013}. Each star is chemically tagged by the local gas abundances, and assigned stellar properties from a grid of stellar evolution tracks \citep{Bressan2012}, which ultimately govern the radiative properties of the stars (if radiation feedback is used), the stellar wind velocity, and the star's lifetime. We follow the mass loss of each particle via stellar winds and supernovae over time as determined by our stellar yield model.
...
\begin{table}
 ...
 \hline
 \multicolumn{1}{|m{1.5cm}|}{Model Number} & \multicolumn{1}{m{1.2cm}|}{SN and Winds} & \multicolumn{1}{m{1.75cm}|}{Ionizing Radiation} & \multicolumn{1}{m{1.25cm}|}{PE Heating} & Resolution (pc) & 10$^{3}$ SU/Myr & Time (Gyr) & \multicolumn{1}{m{1.0cm}|}{Total SU (10$^{6}$)} \\
 \hline

  1 & Y  & N & N & 1.5 & 0.24 & 1.0 & 0.24 \\
  2 & Y* & N & N & 1.5 & 0.12 & 1.0 & 0.12 \\
  3 & Y  & Y & N & 1.5 & 2.4  & 1.0 & 2.4 \\
  4 & Y  & N & N & 1.5 & 1.2  & 1.0 & 1.2 \\
  5 & Y  & N & Y & 1.5 & 0.24 & 1.0 & 0.24 \\
  6 & Y  & Y & Y & 1.5 & 1.2  & 1.0 & 1.2 \\
  7 & Y  & Y & Y & 1.5 & 3.0  & 1.0 & 3.0 \\
  8 & Y  & Y & Y & 0.75 & 12 & 0.1 & 1.2  \\
  9 & Y  & Y & Y & 3.0 & 0.75 & 0.1 & 0.08  \\
  \hline
  Total & - & - & - & - & - & - & 9.68  \\
 \hline
 \end{tabular}
...
\caption{\small Scaling properties for radiative particles (left) and strong scaling (right) in \texttt{Enzo}. We anticipate a factor of several to 10 slow down with typical radiating particle counts in production runs in both \texttt{Enzo} and \texttt{FLASH}. Our dwarf galaxy models in \texttt{Enzo} scale well to about 128 cores; we anticipate running at 128 cores.}
