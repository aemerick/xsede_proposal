...
\includegraphics[width=0.34\linewidth]{weak_fixed}
\caption{\small Scaling properties for radiative particles (left) and strong scaling (middle) in \texttt{Enzo} and weak scaling in \texttt{FLASH} (right). We anticipate an factor of several to 10 slow down with typical radiating particle counts in production runs in both \texttt{Enzo} and \texttt{FLASH}. Our dwarf galaxy models in \texttt{Enzo} scale well to about 128 cores; we anticipate running at 128 cores. The right panel demonstrates scaling of the AMUSE/Flash run up to 562 cores.}
